...
/* Las funciones anidadas son una caracteristica extendida de C que provee *
 ...
 * o que no se puedan hacer igualmente usando C estandar. En otros         *
...
int main(void) {
    ...
    // Quizas de las pocas cosas interesantes que se pueden hacer por medio
    ...
    // llamar la función el resultado es practicamente el mismo.
    int resultado = funcion_anidada1(({
        // Función más anidada aún, está declarada justo al pasarle el
        // argumento a 'funcion_anidada1'.
        int hola(void) {
            printf("Esta es una funciOn declarada donde es usada"
                   ", dejarA de existir en la siguiente lInea");
        } hola;
    }));

    printf("El resultado de la funciOn anidada es: %d\n", resultado);
...
void funcion_normal(void) {
    printf("Si se descomenta la siguiente lInea habrA un error de compilaciOn");
