/* =======================================*
 * Clase C PROTECO                        *
 * Recursividad aplicada a cadenas        *
 * DescripciOn:                           *
 *  Manejo de arreglos dentro de c99 y c11*
 * ====================================== */
...
int main(void) {
    ...
    printf("La longitud de la cadena es: %d\n", longitud(cadena) - 1); // Se le resta uno para no contar el salto de lInea
...
void imprimir(char * cadena, _Bool direccion) {
    // Una vez que llega al carActer nulo simplemente regresa
    if (*cadena == '\0') return;

    if ( direccion == NORMAL ) {
        // Primero imprime el carActer actual y luego el resto de la cadena
        putchar(*cadena);
        imprimir(cadena + 1, NORMAL);
    } else if ( direccion == REVERSA ) {
        // Primero imprime el resto de la cadena y luego el carActer actual
