...
int main(int argc, const char ** argv) {
    ...
    if ( argc == 3 ) {
        ...
        // Se revisa que los argumentos solo contengan numeros ( y tambiEn
        ...
        for (int arg = 1; arg <= 2; arg++) {
            ...
            for (int i = 0; argv[arg][i]; i++) {
                if ( !isdigit(argv[arg][i]) )
                    imprimir_error("Argumento invAlido");
            }
        }
        ...
        if ( !scanf("%d", &numero) ) {
            puts("Formato de nUmero incorrecto");
            exit(EXIT_FAILURE);
        }
        ...
        if ( !scanf("%d", &base) ) {
            fprintf(stderr, "Formato de nUmero incorrecto");
            exit(EXIT_FAILURE);
