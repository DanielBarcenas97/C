...
int main(void) {
    ...
    puts("Los enteros tiene 5 operadores aritméticos:\n"
            ...
            "\t2. resta -> -\n"
            "\t3. multiplicaciOn -> *\n"
            "\t4. division entera (trunca resultados) -> /\n"
            "\t5. residuo -> %\n");
    ...
    if ( !exito ) // Si algUn nUmero se ingresO erroneamente
        imprime_error("Los números ingresados no son válidos");

    // Si se puede dividir por 0 en aritmEtica flotante por lo que
    // no se pone ninguna restricciOn para el denominador.
    ...
    // Igualmente se estan usando dos especificadores de formato pero se
    // provee de una sola variable al printf. Si ves resultados correctos
    // seguramente estas alucinando. Quizas leer sobre argumentos variables
    // en C te aclare que está pasando y porque no deberias usar esta forma
