/* =======================================*
 * Clase C PROTECO                        *
 * DeclaraciOn y uso de arreglos          *
 * DescripciOn:                           *
 *  Manejo de arreglos dentro de c99      *
 * ====================================== */
...
void recibe_arreglo(short arreglo[]);
...
int main(void) {
    ...
    // No indicamos el tamanio, el compilador cuenta los elementos y asigna el tamanio justo
    // El resultado es un arreglo con justo 8 letras sin un caracter de terminaciOn de cadena
    char arreglo2[] = { 'Q', 'u', 'e', ' ', 'b', 'i', 'e', 'n', '!' };
    printf("Tamanio del arreglo de caracteres: %z\n", TAMANIO_ARREGLO(arreglo2));

    printf("El arreglo de caracteres impreso mediante un ciclo: ");
    RECORRIDO_ARREGLO(arreglo2) putchar(arreglo2[i]);

    // La siguiente impresiOn puede contener caracteres basura
    printf("\nEl arreglo impreso mediante el especificador de conversiOn de cadenas: %s\n", arreglo2);
    ...
    // ocurre un error de compilaciOn, para observaro descomente la siguiente lInea.
    //_Bool arreglo4[];
    ...
    // expresiOn/funcion que reciba alguno de los mismo. MAs allA de la lInea en que son
    // usados no se les podrA volver a referenciar.
    recibe_arreglo( (short[]) { 0b10, 0b100, 0b1000, 0b10000 } );
    ...
    long arreglo_longitud_variable[tam];

    puts("El arreglo reservado dinamicamente es:");
    RECORRIDO_ARREGLO(arreglo_longitud_variable)
        printf("%ld", arreglo_longitud_variable[i]);

    // Al tratar con arreglos multidimensionales tambiEn hay varias formas de inicializaciOn
...
longitud.
*/
void recibe_arreglo(short arreglo[4]) {
